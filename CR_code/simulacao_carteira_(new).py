...
def read_selected_funds():
    ...
    try:
        ...
        flag_MFO = 1
        print("Reading 'Carteira Recomendada - MFO.xlsm'.")
        # File is already open
    except Exception:
        print("Reading 'Carteira Recomendada - AAI.xlsm'.")
    ...
    if flag_MFO == 1:
        ...
        portfolio = portfolio[['Ativo','CNPJ', 'Custódia', 'R$ / $', '% do PL', 'Benchmark', '% Benchmark', 'Benchmark +', 'Liquidez/ Duration', 'Classe', 'Estratégia']]
    ...
    additional_list = ['CDI', 'IPCA', 'Prévia IPCA', 'SELIC', 'Ibovespa']
    benchmark_list = list(set(benchmark_list + additional_list))
...
def calculate_portfolio_returns(portfolio, assets_returns, fund_Returns, stock_Returns, fixedIncome_Returns, benchmark_Returns, cnpj_list, stocks_list, fixedIncome_list, flag_MFO, taxa_gestao, benchmark, flag_fixedIncome_MtM):
        ...
        if portfolio.loc[asset, '% Benchmark'] != 0:
            if portfolio.loc[asset, 'Benchmark +'] != 0:
                assets_returns.loc[assets_returns[asset].isna(), asset] = (
                    1 + benchmark_Returns.loc[assets_returns[asset].isna(), portfolio.loc[asset, 'Benchmark']] *
                    portfolio.loc[asset, '% Benchmark']) * (
                                                                               1 + portfolio.loc[asset, 'Benchmark +']) ** (
                                                                               1 / 252) - 1
            else:
                assets_returns.loc[assets_returns[asset].isna(), asset] = benchmark_Returns.loc[
                                                                              assets_returns[asset].isna(),
                                                                              portfolio.loc[asset, 'Benchmark']] * \
                                                                          portfolio.loc[asset, '% Benchmark']
        elif portfolio.loc[asset, 'Benchmark +'] != 0:
            if portfolio.loc[asset, 'Benchmark'] != "-" and portfolio.loc[asset, 'Benchmark'] != 0:
                assets_returns.loc[assets_returns[asset].isna(), asset] = (
                    1 + benchmark_Returns.loc[assets_returns[asset].isna(), portfolio.loc[asset, 'Benchmark']]) * (
                                                                               1 + portfolio.loc[asset, 'Benchmark +']) ** (
                                                                               1 / 252) - 1
            else:
                assets_returns.loc[assets_returns[asset].isna(), asset] = (1 + portfolio.loc[asset, 'Benchmark +']) ** (
                            1 / 252) - 1
        elif portfolio.loc[asset, 'Benchmark'] != "-":
            assets_returns.loc[assets_returns[asset].isna(), asset] = benchmark_Returns.loc[
                assets_returns[asset].isna(), portfolio.loc[asset, 'Benchmark']]
    ...
    strategy_weights.index = [sub.replace('Pós-fixado', 'RF Pós').replace('Pré-fixado', 'RF Pré').replace('Inflação', 'RF Inflação')
                              .replace('Macro', 'MM Macro').replace('Descorrelacionados', 'MM Descorrelados') for sub in strategy_list]
    ...
    if flag_MFO == 1:
        ...
        vehicle = pd.Series(data=list(portfolio['Custódia'].unique()), index=list(portfolio['Custódia'].unique()))
        vehicle = vehicle.apply(lambda x: portfolio[(portfolio['Custódia'] == x)]['% do PL'].sum()).astype(float)

    print("Done.")

    return assets_returns, portfolio_return, portfolio_acc, strategy_weights, strategy_weightsAll, strategy_returns, strategy_attr, strategy_acc, class_weights, class_weightsAll, class_returns, class_attr, class_acc, vehicle

''' 7) CALCULATE PERFORMANCE METRICS --------------------------------------------------------------------------------------------------------------------------------'''

def calculate_correlation_matrix(assets_returns, benchmark_Returns, benchmark, portfolio, cnpj_list):
    # Adicionar CDI como a primeira coluna para a comparação
    asset_group = [benchmark]
    
    # Incluir apenas os ativos com CNPJ válido
...
def print_to_excel(file_name, portf_vs_bench_1, portf_vs_bench_2, portf_vs_bench_3, portf_vs_bench_4, portf_vs_bench_5, portf_vs_bench_6, class_perf_attr, strategy_perf_attr, class_weights, strategy_weightsAll, vehicle, class_weightsAll, portfolio_acc, volatility, correlation, drawdown, assets_returns, strategy_acc):
    # Create workbook object (try to open an existing one, if it doesn`t exist, create one)
    try:
...
def main_code():
    ...
    portfolio, fund_prices, fixedIncome_prices, stock_prices, benchmark_prices = import_fund_prices(portfolio, date_first, date_last, flag_fixedIncome_MtM, cnpj_list, fixedIncome_list, stocks_list, benchmark_list, dict_CNPJ)
    portfolio = manipulate_portfolio_categorical_columns(portfolio)
    fund_Returns, fixedIncome_Returns, stock_Returns, benchmark_Returns = get_assets_and_benchmarks_daily_returns(portfolio, date_first, date_last, flag_fixedIncome_MtM, cnpj_list, fixedIncome_list, stocks_list, benchmark_list, dict_CNPJ, fund_prices, fixedIncome_prices, stock_prices, benchmark_prices)
    assets_returns, portfolio_return, portfolio_acc, strategy_weights, strategy_weightsAll, strategy_returns, strategy_attr, strategy_acc, class_weights, class_weightsAll, class_returns, class_attr, class_acc, vehicle = calculate_portfolio_returns(portfolio, pd.DataFrame(), fund_Returns, stock_Returns, fixedIncome_Returns, benchmark_Returns, cnpj_list, stocks_list, fixedIncome_list, flag_MFO, taxa_gestao, benchmark, flag_fixedIncome_MtM)    
    ...
    file_name = 'Carteira Recomendada - MFO.xlsm'  # Definindo o nome do arquivo
    volatility = pd.concat([portfolio_return, benchmark_Returns[benchmark]], axis=1)
    volatility.columns = ["Portfólio Modelo", benchmark]
    ...
    print_to_excel(file_name, portf_vs_bench_1, portf_vs_bench_2, portf_vs_bench_3, portf_vs_bench_4, portf_vs_bench_5, portf_vs_bench_6, class_perf_attr, strategy_perf_attr, class_weights, strategy_weightsAll, vehicle, class_weightsAll, portfolio_acc, volatility, correlation, drawdown, assets_returns, strategy_acc)
